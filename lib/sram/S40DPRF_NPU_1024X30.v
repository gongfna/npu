...
//
//       Instance Name:              S40DPRF_NPU_1024X30
//       Words:                      1024
//       Bits:                       30
//       Mux:                        4
//       Drive:                      6
...
`ifdef POWER_PINS
module S40DPRF_NPU_1024X30 (VDDCE, VDDPE, VSSE, QA, CLKA, CENA, AA, CLKB, CENB, AB,
    DB, EMAA, EMAB, RET1N, COLLDISN);
`else
module S40DPRF_NPU_1024X30 (QA, CLKA, CENA, AA, CLKB, CENB, AB, DB, EMAA, EMAB, RET1N,
    COLLDISN);
`endif
 ...
 wire  CLKB_;
  ...
  buf B91(COLLDISN_, COLLDISN);
   ...
   `ifdef ARM_FAULT_MODELING
     S40DPRF_NPU_1024X30_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .Q_in(QA_int));
  `else
...
`endif
`timescale 1ns/1ps
module S40DPRF_NPU_1024X30_error_injection (Q_out, Q_in, CLK, A, CEN);
   output [29:0] Q_out;
   input [29:0] Q_in;
   input CLK;
   input [9:0] A;
