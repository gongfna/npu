...
//
//       Instance Name:              S40SPRF_NPU_1024X19
//       Words:                      1024
//       Bits:                       19
//       Mux:                        4
//       Drive:                      6
...
//       
//       Creation Date:  Fri Jul 13 09:51:12 2018
//       Version: 	r1p1
//
...
`ifdef POWER_PINS
module S40SPRF_NPU_1024X19 (VDDCE, VDDPE, VSSE, Q, CLK, CEN, WEN, A, D, EMA, EMAW, GWEN, RET1N);
`else
module S40SPRF_NPU_1024X19 (Q, CLK, CEN, WEN, A, D, EMA, EMAW, GWEN, RET1N);
`endif
  ...
  parameter MUX = 4;
  parameter MEM_WIDTH = 76; // redun block size 4, 36 on left, 40 on right
...
`endif
 ...
 wire  CLK_;
  ...
  assign `ARM_UD_SEQ Q_ = (RET1N_ | pre_charge_st) ? ((Q_int)) : {19{1'bx}};
    ...
    if (RET1N_int === 1'bx || RET1N_int === 1'bz) begin
      ...
      if( isBitX(GWEN_int) ) begin
        ...
        D_int = {19{1'bx}};
          ...
          writeEnable = ~ ( {19{GWEN_int}} | {WEN_int[18], WEN_int[17], WEN_int[16],
          WEN_int[15], WEN_int[14], WEN_int[13], WEN_int[12], WEN_int[11], WEN_int[10],
          WEN_int[9], WEN_int[8], WEN_int[7], WEN_int[6], WEN_int[5], WEN_int[4], WEN_int[3],
          WEN_int[2], WEN_int[1], WEN_int[0]});
      if (GWEN_int !== 1'b1) begin
        row_mask =  ( {3'b000, writeEnable[18], 3'b000, writeEnable[17], 3'b000, writeEnable[16],
        ...
        new_data =  ( {3'b000, D_int[18], 3'b000, D_int[17], 3'b000, D_int[16], 3'b000, D_int[15],
          3'b000, D_int[14], 3'b000, D_int[13], 3'b000, D_int[12], 3'b000, D_int[11],
          3'b000, D_int[10], 3'b000, D_int[9], 3'b000, D_int[8], 3'b000, D_int[7],
          3'b000, D_int[6], 3'b000, D_int[5], 3'b000, D_int[4], 3'b000, D_int[3], 3'b000, D_int[2],
          3'b000, D_int[1], 3'b000, D_int[0]} << mux_address);
        row = (row & ~row_mask) | (row_mask & (~row_mask | new_data));
        mem[row_address] = row;
      end else begin
        data_out = (row >> (mux_address));
        readLatch0 = {data_out[72], data_out[68], data_out[64], data_out[60], data_out[56],
      ...
      if (GWEN_int !== 1'b1) begin
        ...
        Q_int = {shifted_readLatch0[18], shifted_readLatch0[17], shifted_readLatch0[16],
          shifted_readLatch0[15], shifted_readLatch0[14], shifted_readLatch0[13], shifted_readLatch0[12],
          shifted_readLatch0[11], shifted_readLatch0[10], shifted_readLatch0[9], shifted_readLatch0[8],
          shifted_readLatch0[7], shifted_readLatch0[6], shifted_readLatch0[5], shifted_readLatch0[4],
          shifted_readLatch0[3], shifted_readLatch0[2], shifted_readLatch0[1], shifted_readLatch0[0]};
      end
      if( isBitX(GWEN_int) ) begin
        Q_int = {19{1'bx}};
      end
...
`endif
  ...
  reg NOT_CEN, NOT_WEN18, NOT_WEN17, NOT_WEN16, NOT_WEN15, NOT_WEN14, NOT_WEN13, NOT_WEN12;
  reg NOT_WEN11, NOT_WEN10, NOT_WEN9, NOT_WEN8, NOT_WEN7, NOT_WEN6, NOT_WEN5, NOT_WEN4;
  reg NOT_WEN3, NOT_WEN2, NOT_WEN1, NOT_WEN0, NOT_A9, NOT_A8, NOT_A7, NOT_A6, NOT_A5;
  reg NOT_A4, NOT_A3, NOT_A2, NOT_A1, NOT_A0, NOT_D18, NOT_D17, NOT_D16, NOT_D15, NOT_D14;
 ...
 wire  CLK_;
  ...
  buf B19(CLK_, CLK);
  buf B20(CEN_, CEN);
  buf B21(WEN_[0], WEN[0]);
  buf B22(WEN_[1], WEN[1]);
  buf B23(WEN_[2], WEN[2]);
  buf B24(WEN_[3], WEN[3]);
  buf B25(WEN_[4], WEN[4]);
  buf B26(WEN_[5], WEN[5]);
  buf B27(WEN_[6], WEN[6]);
  buf B28(WEN_[7], WEN[7]);
  buf B29(WEN_[8], WEN[8]);
  buf B30(WEN_[9], WEN[9]);
  buf B31(WEN_[10], WEN[10]);
  buf B32(WEN_[11], WEN[11]);
  buf B33(WEN_[12], WEN[12]);
  buf B34(WEN_[13], WEN[13]);
  buf B35(WEN_[14], WEN[14]);
  buf B36(WEN_[15], WEN[15]);
  buf B37(WEN_[16], WEN[16]);
  buf B38(WEN_[17], WEN[17]);
  buf B39(WEN_[18], WEN[18]);
  buf B40(A_[0], A[0]);
  buf B41(A_[1], A[1]);
  buf B42(A_[2], A[2]);
  buf B43(A_[3], A[3]);
  ...
  buf B67(D_[17], D[17]);
  buf B68(D_[18], D[18]);
  buf B69(EMA_[0], EMA[0]);
  buf B70(EMA_[1], EMA[1]);
  buf B71(EMA_[2], EMA[2]);
  buf B72(EMAW_[0], EMAW[0]);
  buf B73(EMAW_[1], EMAW[1]);
  buf B74(GWEN_, GWEN);
  buf B75(RET1N_, RET1N);

   `ifdef ARM_FAULT_MODELING
     S40SPRF_NPU_1024X19_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .GWEN(GWEN_int), .WEN(WEN_int), .Q_in(Q_int));
  `else
  assign Q_ = (RET1N_ | pre_charge_st) ? ((Q_int)) : {19{1'bx}};
  `endif
...
`endif
  ...
  wire RET1Neq1, RET1Neq1aCENeq0aWEN18eq0aGWENeq0, RET1Neq1aCENeq0aWEN17eq0aGWENeq0;
...
`endif
`timescale 1ns/1ps
module S40SPRF_NPU_1024X19_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, GWEN);
   output [18:0] Q_out;
   input [18:0] Q_in;
   input CLK;
   input [9:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(10'd754,5'd15,2'd1,2'd0);
   `endif
end
   task add_fault;
