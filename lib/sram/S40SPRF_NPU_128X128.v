...
//
//       Instance Name:              S40SPRF_NPU_128X128
//       Words:                      128
//       Bits:                       128
//       Mux:                        2
//       Drive:                      6
...
`ifdef POWER_PINS
module S40SPRF_NPU_128X128 (VDDCE, VDDPE, VSSE, Q, CLK, CEN, WEN, A, D, EMA, EMAW, GWEN, RET1N);
`else
module S40SPRF_NPU_128X128 (Q, CLK, CEN, WEN, A, D, EMA, EMAW, GWEN, RET1N);
`endif
 ...
 wire  CLK_;
  ...
  buf B399(RET1N_, RET1N);
   ...
   `ifdef ARM_FAULT_MODELING
     S40SPRF_NPU_128X128_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .GWEN(GWEN_int), .WEN(WEN_int), .Q_in(Q_int));
  `else
...
`endif
`timescale 1ns/1ps
module S40SPRF_NPU_128X128_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, GWEN);
   output [127:0] Q_out;
   input [127:0] Q_in;
   input CLK;
   input [6:0] A;
