...
`ifdef POWER_PINS
module rfsp1024x19 (VDDCE, VDDPE, VSSE, Q, CLK, CEN, WEN, A, D, EMA, EMAW, GWEN, RET1N);
`else
module rfsp1024x19 (Q, CLK, CEN, WEN, A, D, EMA, EMAW, GWEN, RET1N);
`endif
 ...
 wire  CLK_;
  ...
  buf B75(RET1N_, RET1N);
   ...
   `ifdef ARM_FAULT_MODELING
     rfsp1024x19_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .GWEN(GWEN_int), .WEN(WEN_int), .Q_in(Q_int));
  `else
...
`endif
`timescale 1ns/1ps
module rfsp1024x19_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, GWEN);
   output [18:0] Q_out;
   input [18:0] Q_in;
   input CLK;
   input [9:0] A;
