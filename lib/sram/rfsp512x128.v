...
//       
//       Creation Date:  Fri Jul 13 14:34:27 2018
//       Version: 	r1p1
//
...
`ifdef POWER_PINS
module rfsp512x128 (VDDCE, VDDPE, VSSE, Q, CLK, CEN, WEN, A, D, EMA, EMAW, GWEN, RET1N);
`else
module rfsp512x128 (Q, CLK, CEN, WEN, A, D, EMA, EMAW, GWEN, RET1N);
`endif
...
`endif
  ...
  reg NOT_WEN1, NOT_WEN0, NOT_A8, NOT_A7, NOT_A6, NOT_A5, NOT_A4, NOT_A3, NOT_A2, NOT_A1;
  reg NOT_A0, NOT_D127, NOT_D126, NOT_D125, NOT_D124, NOT_D123, NOT_D122, NOT_D121;
 ...
 wire  CLK_;
  ...
  buf B365(D_[98], D[98]);
  buf B366(D_[99], D[99]);
  buf B367(D_[100], D[100]);
  buf B368(D_[101], D[101]);
  buf B369(D_[102], D[102]);
  buf B370(D_[103], D[103]);
  buf B371(D_[104], D[104]);
  buf B372(D_[105], D[105]);
  buf B373(D_[106], D[106]);
  buf B374(D_[107], D[107]);
  buf B375(D_[108], D[108]);
  buf B376(D_[109], D[109]);
  buf B377(D_[110], D[110]);
  buf B378(D_[111], D[111]);
  buf B379(D_[112], D[112]);
  buf B380(D_[113], D[113]);
  buf B381(D_[114], D[114]);
  buf B382(D_[115], D[115]);
  buf B383(D_[116], D[116]);
  buf B384(D_[117], D[117]);
  buf B385(D_[118], D[118]);
  buf B386(D_[119], D[119]);
  buf B387(D_[120], D[120]);
  buf B388(D_[121], D[121]);
  buf B389(D_[122], D[122]);
  buf B390(D_[123], D[123]);
  buf B391(D_[124], D[124]);
  buf B392(D_[125], D[125]);
  buf B393(D_[126], D[126]);
  buf B394(D_[127], D[127]);
  buf B395(EMA_[0], EMA[0]);
  buf B396(EMA_[1], EMA[1]);
  buf B397(EMA_[2], EMA[2]);
  buf B398(EMAW_[0], EMAW[0]);
  buf B399(EMAW_[1], EMAW[1]);
  buf B400(GWEN_, GWEN);
  buf B401(RET1N_, RET1N);

   `ifdef ARM_FAULT_MODELING
     rfsp512x128_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .GWEN(GWEN_int), .WEN(WEN_int), .Q_in(Q_int));
  `else
...
`endif
`timescale 1ns/1ps
module rfsp512x128_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, GWEN);
   output [127:0] Q_out;
   input [127:0] Q_in;
   input CLK;
   input [8:0] A;
