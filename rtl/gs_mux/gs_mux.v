...
`XDMA_IO_IN2(LSTMB, lstmb)
`XDMA_IO_IN2(WB, wb)
`XDMA_IO_IN2(WIB, wib)
`XDMA_IO_IN2(INSTB, instb)
`XDMA_IO_IN2(BIASB, biasb)
`NPE_IO_OUT(IOB0, iob0)
`NPE_IO_OUT(IOB1, iob1)
`NPE_IO_OUT2(LSTMB, lstmb)
`NPE_IO_OUT2(WB, wb)
`NPE_IO_OUT2(WIB, wib)
`NPE_IO_OUT2(INSTB, instb)
`NPE_IO_OUTEND(BIASB, biasb)
);

wire buffer_debug;
assign buffer_debug = axi_wr_buf_en;

`MUX_LOGIC(gs1, iob0)
`MUX_LOGIC(gs2, iob1)
`MUX_LOGIC2(gs3, lstmb)
`MUX_LOGIC2(gs4, wb)
`MUX_LOGIC2(gs5, wib)
`MUX_LOGIC2(gs6, instb)
`MUX_LOGIC2(gs7, biasb)
