...
//----------------------------------------------------------------------
  ...
  parameter PL_ARB = 0; // Set to 1 if arb. output should be pipelined.
  ...
  parameter BUS_PRIORITY_W = 8; // Width of bus containing priorities of
                                // all visible masters.

  parameter ARB_BURST_LOCK = 0; // Lock arbitration to client until 
                                // burst completes.
...
// LOCAL MACROS.
//----------------------------------------------------------------------
  `define ARB_PARK_MODE    0 // Macro for park mode parameter of
  ...
  `define BUS_PRIOR_W_INTERNAL (`NC_INTERNAL*PRIORITY_W)
...
// Inputs - System.
   ...
   input [BUS_PRIORITY_W-1:0] bus_priorities_i; // Bus containing 
                                                // priorities of all 
	         			                                // connected ports.
...
// Outputs - Channel Destination.
  ...
  wire [BUS_PRIORITY_W-1:0] bus_priorities_inv; 
  ...
  assign use_other_pri_int = (NC==1) ? 2'b0 : use_other_pri_i;
  assign bus_grant_lock_int = (NC==1) ? 2'b0 : bus_grant_lock_i;

  wire [(`NC_INTERNAL*ARB_INDEX_W)-1:0] bus_priority_other_int;
  wire [(`NC_INTERNAL*ARB_INDEX_W)-1:0] bus_priority_o_int;

  assign bus_priority_other_int = (NC==1) 
                                  ? 2'b0 
                                  : {(`NC_INTERNAL*ARB_INDEX_W){1'b0}};
  ...
  generate 
    case (ARB_TYPE)
      ...
      `AXI_ARB_TYPE_2T : begin : gen_arb_type_2t
        ...
        #(`NC_INTERNAL,       // Number of clients.
	        PRIORITY_W,         // Priority width per client.
          `ARB_PARK_MODE,     
          `ARB_PARK_INDEX,    
          `ARB_OP_MODE_NOREG,
	        LOG2_NC,            // Index width.
