...
//----------------------------------------------------------------------
  ...
  // WIRE VARIABLES.
  //--------------------------------------------------------------------
  wire [MAX_UIDA-1:0] bus_valid_uida; // Valid signals selected by 
                                      // the active slave numbers for
				                              // each unique ID value.
  
  // Priority signals to the mux selected by the active slave numbers
  // for each unique ID value allowed.
  wire [`AXI_SLV_PRIORITY_W*MAX_UIDA-1:0] bus_priority_uida; 
  ...
  wire [`AXI_MAX_NUM_MST_SLVS-1:0] has_ddctd_m_bus_max;
  // Above sized for NSM.
  ...
  // Create a register slice for bus_grant to match with irs_arbpl 
  // pipeline stage.
  reg [NUM_VIS_SP-1:0] bus_grant_rs_arbpl;
  reg [NUM_VIS_SP-1:0] bus_grant_rs_arbpl_r;
