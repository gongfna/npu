...
//----------------------------------------------------------------------
  ...
  parameter WCH = 0; // Set to 1 if the block is implementing a write
                     // channel i.e. write address channel.

  // Does this dedicated channel, have a dedicated link.
  parameter HAS_SHRD_DDCTD_LNK = 0;
...
//----------------------------------------------------------------------
  ...
  wire issued_tx; // Asserted when a transaction is issued in this
                  // channel.
  ...
  //--------------------------------------------------------------------
  reg valid_granted_mca_r; // Registered version of valid_granted_mca.			  

  /*--------------------------------------------------------------------
  * Shared Channel Request Masking Signals.
  */

  // Transaction count register per attached slave.
  reg [LOG2_FAC_P1_S0-1:0] tx_cnt_s0;
  reg [LOG2_FAC_P1_S1-1:0] tx_cnt_s1;
  reg [LOG2_FAC_P1_S2-1:0] tx_cnt_s2;
  reg [LOG2_FAC_P1_S3-1:0] tx_cnt_s3;
  reg [LOG2_FAC_P1_S4-1:0] tx_cnt_s4;
  reg [LOG2_FAC_P1_S5-1:0] tx_cnt_s5;
  reg [LOG2_FAC_P1_S6-1:0] tx_cnt_s6;
  reg [LOG2_FAC_P1_S7-1:0] tx_cnt_s7;
  reg [LOG2_FAC_P1_S8-1:0] tx_cnt_s8;
  reg [LOG2_FAC_P1_S9-1:0] tx_cnt_s9;
  reg [LOG2_FAC_P1_S10-1:0] tx_cnt_s10;
  reg [LOG2_FAC_P1_S11-1:0] tx_cnt_s11;
  reg [LOG2_FAC_P1_S12-1:0] tx_cnt_s12;
  reg [LOG2_FAC_P1_S13-1:0] tx_cnt_s13;
  reg [LOG2_FAC_P1_S14-1:0] tx_cnt_s14;
  reg [LOG2_FAC_P1_S15-1:0] tx_cnt_s15;
  reg [LOG2_FAC_P1_S16-1:0] tx_cnt_s16;
  ...
  U_DW_axi_sp_lockarb (
    ...
    .aclk_i                (aclk_i),
    .aresetn_i             (aresetn_i),
    .bus_mst_priorities_i  (bus_mst_priorities_i),
  ...
  // leda FM_2_7 off
  // FM_2_7: Use named association in port map
  // leda I_1C_R off
  // I_1C_R: Map module ports in instantiation by named association, not by
  // position
  // leda W416 off
  // W416: Instance connections not by name
  // These 3 leda warnings (FM_2_7, I_1C_R, W416) all warning about
  // the following instances not using named port mapping. Since the
  // module in question is trivial and defined in the same .v file
  // this is not seen as an issue for readability.
  generate
    if(SHARED & (NSS >= 1)) begin : gen_tx_cnt_nss1
      DW_axi_sp_addrch_tx_cnt
       
      #(LOG2_FAC_P1_S0, FAC_S0, (PL_ARB == 0)) U_tx_cnt0
      ( /* In  */ aclk_i, aresetn_i, tx_acc_s_bus_max[0],
                  cpl_tx_r_bus_max[0], 
        /* Out */ max_fac_s0, tx_cnt_nz_s0
      );
    end
    if(SHARED & (NSS >= 2)) begin : gen_tx_cnt_nss2
      ...
      ( /* In  */ aclk_i, aresetn_i, tx_acc_s_bus_max[1],
                  cpl_tx_r_bus_max[1], 
        /* Out */ max_fac_s1, tx_cnt_nz_s1
      );
    end
    if(SHARED & (NSS >= 3)) begin : gen_tx_cnt_nss3
      ...
      ( /* In  */ aclk_i, aresetn_i, tx_acc_s_bus_max[2],
                  cpl_tx_r_bus_max[2], 
        /* Out */ max_fac_s2, tx_cnt_nz_s2
      );
    end
    if(SHARED & (NSS >= 4)) begin : gen_tx_cnt_nss4
      ...
      ( /* In  */ aclk_i, aresetn_i, tx_acc_s_bus_max[3],
                  cpl_tx_r_bus_max[3], 
        /* Out */ max_fac_s3, tx_cnt_nz_s3
      );
    end
    if(SHARED & (NSS >= 5)) begin : gen_tx_cnt_nss5
      ...
      ( /* In  */ aclk_i, aresetn_i, tx_acc_s_bus_max[4],
                  cpl_tx_r_bus_max[4], 
        /* Out */ max_fac_s4, tx_cnt_nz_s4
      );
    end
    if(SHARED & (NSS >= 6)) begin : gen_tx_cnt_nss6
      ...
      ( /* In  */ aclk_i, aresetn_i, tx_acc_s_bus_max[5],
                  cpl_tx_r_bus_max[5], 
        /* Out */ max_fac_s5, tx_cnt_nz_s5
      );
    end
    if(SHARED & (NSS >= 7)) begin : gen_tx_cnt_nss7
      ...
      ( /* In  */ aclk_i, aresetn_i, tx_acc_s_bus_max[6],
                  cpl_tx_r_bus_max[6], 
        /* Out */ max_fac_s6, tx_cnt_nz_s6
      );
    end
    if(SHARED & (NSS >= 8)) begin : gen_tx_cnt_nss8
      ...
      ( /* In  */ aclk_i, aresetn_i, tx_acc_s_bus_max[7],
                  cpl_tx_r_bus_max[7], 
        /* Out */ max_fac_s7, tx_cnt_nz_s7
      );
    end
    if(SHARED & (NSS >= 9)) begin : gen_tx_cnt_nss9
      ...
      ( /* In  */ aclk_i, aresetn_i, tx_acc_s_bus_max[8],
                  cpl_tx_r_bus_max[8], 
        /* Out */ max_fac_s8, tx_cnt_nz_s8
      );
    end
    if(SHARED & (NSS >= 10)) begin : gen_tx_cnt_nss10
      ...
      ( /* In  */ aclk_i, aresetn_i, tx_acc_s_bus_max[9],
                  cpl_tx_r_bus_max[9], 
        /* Out */ max_fac_s9, tx_cnt_nz_s9
      );
    end
    if(SHARED & (NSS >= 11)) begin : gen_tx_cnt_nss11
      ...
      ( /* In  */ aclk_i, aresetn_i, tx_acc_s_bus_max[10],
                  cpl_tx_r_bus_max[10], 
        /* Out */ max_fac_s10, tx_cnt_nz_s10
      );
    end
    if(SHARED & (NSS >= 12)) begin : gen_tx_cnt_nss12
      ...
      ( /* In  */ aclk_i, aresetn_i, tx_acc_s_bus_max[11],
                  cpl_tx_r_bus_max[11], 
        /* Out */ max_fac_s11, tx_cnt_nz_s11
      );
    end
    if(SHARED & (NSS >= 13)) begin : gen_tx_cnt_nss13
      ...
      ( /* In  */ aclk_i, aresetn_i, tx_acc_s_bus_max[12],
                  cpl_tx_r_bus_max[12], 
        /* Out */ max_fac_s12, tx_cnt_nz_s12
      );
    end
    if(SHARED & (NSS >= 14)) begin : gen_tx_cnt_nss14
      ...
      ( /* In  */ aclk_i, aresetn_i, tx_acc_s_bus_max[13],
                  cpl_tx_r_bus_max[13], 
        /* Out */ max_fac_s13, tx_cnt_nz_s13
      );
    end
    if(SHARED & (NSS >= 15)) begin : gen_tx_cnt_nss15
      ...
      ( /* In  */ aclk_i, aresetn_i, tx_acc_s_bus_max[14],
                  cpl_tx_r_bus_max[14], 
        /* Out */ max_fac_s14, tx_cnt_nz_s14
      );
    end
    if(SHARED & (NSS >= 16)) begin : gen_tx_cnt_nss16
      ...
      ( /* In  */ aclk_i, aresetn_i, tx_acc_s_bus_max[15],
                  cpl_tx_r_bus_max[15], 
        /* Out */ max_fac_s15, tx_cnt_nz_s15
      );
    end
    if(SHARED & (NSS >= 17)) begin : gen_tx_cnt_nss17
      ...
      ( /* In  */ aclk_i, aresetn_i, tx_acc_s_bus_max[16],
                  cpl_tx_r_bus_max[16], 
        /* Out */ max_fac_s16, tx_cnt_nz_s16
      );
    end
  endgenerate
  // leda FM_2_7 on
  ...
  assign tx_cnt_nz_target_slv 
    = ~|(tx_cnt_nz_s_bus & bus_valid_shrd_masked);
  ...
  */
  ...
  assign issued_mstnum_o 
    = (NUM_CON_MP != NUM_VIS_MP) 
      ? {LOG2_NUM_VIS_MP{1'b0}} 
      : grant_m_local;
...
module DW_axi_sp_addrch_tx_cnt (
  ...
  parameter REG_OUTPUT = 1; // Registered or combinatorial output.
  ...
  assign cnt_max_o = (REG_OUTPUT ? txcount_r : txcount) == CNT_MAX;
