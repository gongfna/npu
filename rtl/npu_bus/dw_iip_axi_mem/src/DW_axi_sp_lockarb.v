...
//----------------------------------------------------------------------
  ...
  reg  [LOG2_NC-1:0]      grant_m_local_arb_r;    
  ...
  begin : gen_req_arb_PROC
    ...
    end else begin // if(`IMPLEMENT_LOCKING)
		  req_arb = (AWCH==1) ? wreq_i : rreq_i;
  ...
  begin : txcount_PROC
    ...
    // during synthesis.
    txcount = SHARED ? {LOG2_MAX_FAC_P1{1'b0}} : txcount_r;
  ...
  always @(*) begin : use_other_pri_PROC
    ...
    // locking client.
    if(((ARB_TYPE==1) | (ARB_TYPE==2)) & LOCKING) begin
