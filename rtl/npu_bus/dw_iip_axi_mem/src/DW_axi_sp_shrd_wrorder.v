...
//----------------------------------------------------------------------
  ...
  // Leda bug, if this module is instantiated this is a valid selection.
  ...
  begin : tx_started_s_r_PROC
    ...
    // leda S_4C_R off
    // leda W410 off
    // DFT_021: Latch inferred for tx_started_s_r(or part of it)
    // S_4C_R: Latch inferred for tx_started_s_r(or part of it)
    // W410: Latch inferred for tx_started_s_r(or part of it)
    // Leda bug, DC does not find latches here.
    if(~aresetn_i) begin
  ...
  always @(*) begin : mst_req_allow_bus_PROC
    integer slv;
    integer mbit;
    reg [L2_NM:0] mst; // Note extra bit, so in compilation "mst" can
                       // always reach loop exit condition. Extra bit
      ...
      for(slv=0 ; slv<=(NS-1) ; slv=slv+1) begin
        ...
        mst_req_allow_bus[mst] 
          =   mst_req_allow_bus[mst] 
                   ...
                   // DCVER_256: Illegal part selection
                   // E267: Range index out of bound
                   // L2_NM will never be > log base 2 NM.
