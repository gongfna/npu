...
// Dummy wires - used to suppress unconnected ports warnings by Leda tool - BR - 2/24/2010
  ...
  begin : rsif_asize_mp_bytes_PROC
    case (rsif_asize_mp)
      ...
      // For upsizing configurations an MP_DW of 512 can not
      // be used, so turn off coverage for that
      //VCS coverage off
...
// leda W396 on
  ...
  // UPSIZING 
  // - Upsized t/x signal.
  // - Byte enables and data in.
  // - Data and strobe packing registers.
  // - Data and strobe select muxes.
  //
  // Data and strobe packing registers are associated with a particular
  // RSI slot i.e. RSI slot 0 is associated with data and strobe packing
  // registers 0, and so on. So if an incoming ID matches with a
  // particular RSI slot (say slot 2) then its data will be packed in
  // packing register 2. Similarly with strobes.
  //
  // The output data and strobes will be driven from *_nxt. This is so
  // we can issue the upsized data beat on the same cycle that we get
  // the final MP beat for each SP beat.
  //--------------------------------------------------------------------
  
  // Generate signal to tell us when the beat from the channel fifo is
  // from an upsized transaction.
  assign upsized_tx = HAS_TX_UPSIZE 
                      ? (mux_asize_mp < mux_asize_sp) 
		      : 1'b0;

  // Since we can do upsizing with any size from the MP, we generate
  // byte enables from the MP asize and current beat address to tell
  // us what bytes to save in our data and strobe packing registers.
  
  // Generate a byte enable for MP data width from mp asize bytes.
  assign size_mp_ben = ~({SP_SW{1'b1}} << mux_asize_mp_bytes);

  // Expand size_mp_ben to SP data width by shifting with current
  // beat address. Note address has been aligned w.r.t. master port
  // size to allow this to work.
  assign size_mp_ben_spw = size_mp_ben << current_addr;

  // Expand MP wdata and strobes by the resizing ratio. 
  assign dpack_wd = {RS_RATIO{et_mp_wdata}};
  assign spack_ws = {RS_RATIO{et_mp_wstrb}};


  // dpack_fixed_wd and spack_fixed_ws are used in the generation of
  // upsized fixed burst t/x's. If a fixed burst is being upsized it 
  // has to be packed from 2 or more fixed mp beats. For a fixed t/x all
  // beats have the same address, so here we take the active bytes on 
  // the master port data bus, and replicate them to all positions in 
  // the slave port data bus. 
  // We use mux_asize_mp to select how many bytes to replicate, NOTE
  // due to upsizing alignment rules they will always be at the start
  // of the mp data bus.
  // The bus_spack_nxt_PROC and bus_dpack_nxt_PROC use a running 
  // address variable to decode where in the sp data bus these bytes 
  // should be packed.
  always @(*)
  begin : dspack_fixed_wd_ws_PROC

    dpack_fixed_wd = {SP_DW{1'b0}};
    spack_fixed_ws = {SP_SW{1'b0}};

    case(mux_asize_mp)
      3'b000 : begin
        dpack_fixed_wd = {SP_DW_DIV8{et_mp_wdata[7:0]}};
        spack_fixed_ws = {SP_DW_DIV8{et_mp_wstrb[0]}};
      end

      3'b001 : begin
        dpack_fixed_wd = {SP_DW_DIV16{et_mp_wdata[15:0]}};
        spack_fixed_ws = {SP_DW_DIV16{et_mp_wstrb[1:0]}};
      end

      3'b010 : begin
        dpack_fixed_wd = {SP_DW_DIV32{et_mp_wdata[31:0]}};
        spack_fixed_ws = {SP_DW_DIV32{et_mp_wstrb[3:0]}};
      end

      3'b011 : begin
        dpack_fixed_wd = {SP_DW_DIV64{et_mp_wdata[63:0]}};
        spack_fixed_ws = {SP_DW_DIV64{et_mp_wstrb[7:0]}};
      end

      3'b100 : begin
        dpack_fixed_wd = {SP_DW_DIV128{et_mp_wdata[127:0]}};
        spack_fixed_ws = {SP_DW_DIV128{et_mp_wstrb[15:0]}};
      end
  ...
  end // dpack_fixed_wd_PROC


  // Write data packing registers, next state process.
  // leda CS_5P_R_A off
  // leda FM_2_15 off
  // leda W336 off
  // leda C_3C_R off
  ...
  // leda S_4C_R off
  // leda W410 off
  // leda thinks the following is a sequencial block
  // and doesn't like the blocking assignments
  always @(*)
  begin : bus_dpack_nxt_PROC

    integer  slot_num;
    integer  byte_num;
    integer  bit_num;

    reg [SP_DW-1:0] mp_wdata_spdw;

    // Have to use a specially mapped version of the MP date bus
    // for upsized fixed t/x's.
    mp_wdata_spdw = (mux_aburst == `X2X_BT_FIXED) 
                    ? dpack_fixed_wd 
		    : dpack_wd;

    bus_dpack_nxt = bus_dpack_r;

   
    // Cycle through RSI slots.
    for(slot_num=0 ; slot_num<=(WID-1) ; slot_num=slot_num+1) begin

      // Is the incoming beat associated with this RSI slot.
      if(rsis_match[slot_num] | rsis_newtx[slot_num]) begin
	
	// Cycle through bytes of SP data bus.
        for(byte_num=0 ;
	    byte_num<=(SP_SW-1) ; 
	    byte_num=byte_num+1
) 
	begin
	  ...
	  // Should we load this byte.
          if(size_mp_ben_spw[byte_num]) begin

	    // Load byte to correct position.
            for(bit_num=0 ;
	        bit_num<=7 ; 
	        bit_num=bit_num+1
) 
	    begin
              bus_dpack_nxt[(slot_num*SP_DW)+(byte_num*8)+bit_num] 
	      = mp_wdata_spdw[(byte_num*8)+bit_num]; 
	    end
  ...
  // Register process for bus_dpack_nxt.
  always @(posedge aclk_i or negedge aresetn_i)
  begin : bus_dpack_r_PROC
    if(~aresetn_i) begin
      bus_dpack_r <= {BUS_SP_DW{1'b0}};
    end else begin
      bus_dpack_r <= bus_dpack_nxt;
    end
  end // bus_dpack_r_PROC
  

  // Write strobe packing registers, next state process.
  // leda CS_5P_R_A off
  // leda FM_2_15 off
  // leda W336 off
  // leda C_3C_R off
  ...
  // and doesn't like the blocking assignments
  always @(*)
  begin : bus_spack_nxt_PROC

    integer  slot_num;
    integer  byte_num;

    reg [SP_SW-1:0] mp_wstrb_spdw;

    // Have to use a specially mapped version of the MP strobe bus
    // for upsized fixed t/x's.
    mp_wstrb_spdw = (mux_aburst == `X2X_BT_FIXED) 
                    ? spack_fixed_ws 
		    : spack_ws;


    bus_spack_nxt = bus_spack_r;
	...
	begin
	  ...
	  // Should we load this strobe.
          if(size_mp_ben_spw[byte_num]) begin

	    // Load strobe.
            bus_spack_nxt[(slot_num*SP_SW)+byte_num] 
	    = mp_wstrb_spdw[byte_num]; 

	  end // if(size_mp_ben
	end //  for(byte_num=..
      end // if(rsis_match ...
    end // for(slot_num ...
  end // bus_spack_nxt_PROC
  // leda CS_5P_R_A on
  // leda FM_2_15 on
  // leda W336 on
  // leda C_3C_R on
  ...
  // Register process for bus_spack_nxt.
  always @(posedge aclk_i or negedge aresetn_i)
  begin : bus_spack_r_PROC
    if(~aresetn_i) begin
      bus_spack_r <= {BUS_SP_SW{1'b0}};
    end else begin
      bus_spack_r <= bus_spack_nxt;
    end
  end // bus_spack_r_PROC


  // Asserted when the next valid data from the channel fifo will fill
  // the packing register and we can issue the upsized beat.
  // When packing mp beats to create upsized beats we will shift
  // the contents of size_mp_ben_spw to use as byte enables to load
  // the bytes we want. When these byte enables reach the MSB position
  // the pack register is almost full and the next beat to pack into
  // the register will fill it.
  assign pack_reg_nxt_full = size_mp_ben_spw[SP_SW-1];


  //--------------------------------------------------------------------
  // For configurations with X2X_WID > 1 and t/x upsizing enabled there
  // is some extra synchronisation we need to do to make sure that we
  // do not break address ordering rules for upsized transactions.
  //
  // For an upsized transaction, we have to tell the write address
  // channel block when we have a fully packed first beat of a t/x ready
  // to issue, the address channel in turn knows it has to wait for 
  // this signal before issuing the upsized t/x from the SP.
  //
  // If the address channel had issued the upsized write t/x before this
  // point, it is possible that a data beat for a different t/x could be
  // issued before the data for the first t/x, and this would break
  // AXI write ordering rules.
  //--------------------------------------------------------------------
  
  //--------------------------------------------------------------------
  // Maintain a register for each write interleaving depth that will
  // tell us when the upsized t/x currently being processed has started.
  //
  // Decoded as a beat of data of an upsized t/x being issued at the
  // slave port, with wlast and ready_i not asserted.
  //--------------------------------------------------------------------
  always @(posedge aclk_i or negedge aresetn_i)
  begin : bus_us_tx_started_r_PROC
    integer slot_num;

    if(!aresetn_i) begin
      bus_us_tx_started_r <= {WID{1'b0}};
    end else begin

      for(slot_num=0 ; slot_num<=(WID-1) ; slot_num=slot_num+1) begin

	if(rsis_match[slot_num] & valid_o & upsized_tx) begin
	    bus_us_tx_started_r[slot_num] <= ~(sp_wlast & ready_i);
	end // if(rsis_update_sp[slot_num]) begin

      end // for(slot_num=0 ; ....

    end // if(!aresetn_i) 
  end // bus_us_tx_started_r_PROC


  // Decode us_issue_req_o
  // Instructs the write address channel to issue the pending upsized
  // write t/x that matches with the id.
  // Only necessary for configs with a non fan out write interleaving 
  // depth > 1, with upsizing enabled, and we are dealing with an 
  // upsized t/x.
  assign us_issue_req_o = 
    ((WID>1) && (HAS_TX_UPSIZE == 1) && upsized_tx)
    ? ( // Do not assert when the resizer is not ready.  
        !us_xact_issue_off_i      

	// Assert when we have a full data beat for an upsized t/x.
        & (pack_reg_nxt_full & match_active_id)

	// Assert if the write data part of the t/x has not already
	// started.
  ...
  //--------------------------------------------------------------------
  // UPSIZING MUXES
  // Select write data and strobes from the packing registers associated
  // with the currently active RSI slot.
  //--------------------------------------------------------------------

  // Write data packing register select mux.
  x256_DW_axi_x2x_busmux_ohsel
  
  #(WID,  // Number of buses to mux between.
  ...
  // Write strobe packing register select mux.
  x256_DW_axi_x2x_busmux_ohsel
  ...
  assign sp_wdata = (HAS_TX_UPSIZE & valid_o)
                    ? (upsized_tx ? sp_us_wdata : sp_nus_wdata)
                    : sp_nus_wdata;

  assign sp_wstrb = (HAS_TX_UPSIZE & valid_o)
                    ? (upsized_tx ? sp_us_wstrb : sp_nus_wstrb)
                    : sp_nus_wstrb;
