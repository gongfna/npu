...
//----------------------------------------------------------------------
  ...
  reg  [BEATS_PER_POP_W-1:0] rsis_fb_bpp; // beat beats per pop. Derived
                                          // from rsis mux_* signals.
...
// Dummy wires - used to suppress unconnected ports warnings by Leda tool - BR - 2/24/2010
  ...
  // compilation errors.
  always @(rsif_bpp or rsif_addr or rsif_asize_mp)
  begin : rsif_fb_bpp_PROC
    reg [ADDR_TRK_W-1:0] addr_bit;
    ...
    begin
      if(addr_bit < rsif_asize_mp) addr[addr_bit] = rsif_addr[addr_bit];
    end
    ...
    // first MP beat.
    if(rsif_asize_mp > MAX_SP_ASIZE) begin
...
// leda W396 on
  ...
  always @(rsis_bpp or mux_addr or mux_asize_mp)
  begin : rsis_fb_bpp_PROC
    ...
    begin
      if(addr_bit < mux_asize_mp) addr[addr_bit] = mux_addr[addr_bit];
    end
  ...
  begin : update_bpp_nxt_PROC
    ...
    if((update_bpp_r == {BEATS_PER_POP_W{1'b0}}) 
       && match_active_id
) begin
